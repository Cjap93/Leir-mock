...
(function (global) {
  ...
  const DEFAULTS = {
    ...
    settings: {
      heroEyebrow: "Lunch · Ullevigatan, Göteborg",
      heroTitleLine1: "Mat lagad",
      heroTitleLine2: "varje vardag.",
      heroButtonText: "Se veckans meny",
      allergenText: "Allergi? Ring oss eller fråga i disken.",
      footerDesc: "Lunchrestaurang mitt i Göteborg. Ny meny varje vardag, alltid lagad från grunden.",
      openDays: "Mån–Fre",
      openTime: "11.00–14.00",
      address1: "Ullevigatan 17–19",
      address2: "411 40 Göteborg",
      mapUrl: "https://maps.google.com/?q=Ullevigatan+17-19+Göteborg",
      phone: "031-30 310 56",
      phoneHref: "0313031056",
      email: "info@leir.nu",
      instagram: "leirmatmoten",
      eveningNote: "Enstaka kvällar — se evenemang",
    },
  ...
  const LeirData = {
    ...
    get(key) {
      const stored = readKey(key, undefined);
      const fallback = DEFAULTS[key];
      if (stored === undefined) return fallback;
      // Shallow-merge objects (not arrays) with defaults so newly added
      // fields (e.g. settings/theme added in a later version) are never
      // missing just because localStorage has an older shape saved.
    ...
    setSettings(v)       { return this.set("settings", v); },
    setTheme(v)          { return this.set("theme", v); },
    ...
    // Call this on every page load so admin theme changes take effect everywhere.
    applyTheme() {
      const theme = this.getTheme();
      const root = document.documentElement.style;
      root.setProperty('--bg', theme.bg);
      root.setProperty('--ink', theme.ink);
      root.setProperty('--rust', theme.rust);
      root.setProperty('--walnut', theme.walnut);
